(function($) {
    ...
    $.widget("mapbender.mbBaseSourceSwitcher", {
        ...
        _setup: function() {
            ...
            /*
            // taking effect only if ul in basesourceswitcher.html.twig is not initially equipped with css class dropdownbs
            if (display === "dropdown") {
        ...
        setHeight: function() {
            ...
            for (var i = 1; i < size; i++) {
                if ($('.mb-element-basesourceswitcher li').eq(i).attr("data-state") === "") {
                    $('.mb-element-basesourceswitcher li').eq(i).css("top", (dis - 40 + heightLi + 3) + "px");
                }
                if (i === 1) {
                    var konst = heightLi;
                }
            ...
            if (display === "dropdown") {
                if (a.attr("data-state") === "active") {
                    if (a.hasClass('isActive')){
                        ...
                        $('.basesourcesetswitch[data-state="active"]').removeClass('isActive');
                    }else{
                        $('.basesourcesetswitch[data-state="active"]').addClass('isActive');
                ...
                this._hideSources();
                me.find('.basesourcesetswitch,.basesourcegroup').not(a).attr('data-state', 'inactive');
                a.attr('data-state', 'active');
                a.parents('.basesourcegroup:first').attr('data-state', 'active').addClass('hidden');
                a.parents('.basesourcesubswitcher:first').addClass('hidden');
                if (a.hasClass('notgroup')) {
