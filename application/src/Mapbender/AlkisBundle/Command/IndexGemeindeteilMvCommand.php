...
class IndexGemeindeteilMvCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
            ->setDescription('Reset\'s the solr index.')
            ->setName('hro:index:reset:mv_gemeindeteile');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln('Indiziere Gemeindeteile fuer MV-Adressensuche ... ');

        $stmt = $conn->query("SELECT count(*) AS count FROM regis.gemeindeteile");
        $result = $stmt->fetch();
        ...
        while ($offset < $result['count']) {
            $stmt = $conn->query("
                ...
                FROM regis.gemeindeteile ORDER BY id LIMIT " . $limit . " OFFSET " . $offset);
            ...
            while ($row = $stmt->fetch()) {
                ...
                $doc->label = "2".$row['gemeinde_name'].$row['gemeindeteil_name'];
