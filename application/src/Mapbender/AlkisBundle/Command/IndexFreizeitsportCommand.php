...
class IndexFreizeitsportCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
        ->setDescription('Reset\'s the solr index.')
        ->setName('hro:index:reset:freizeitsport');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln('Indiziere Freizeitsport fuer HRO-Freizeitsportsuche ... ');


        $stmt = $conn->query('SELECT count(*) AS count FROM fachdaten.freizeitsport_regis_hro');
        $result = $stmt->fetch();
