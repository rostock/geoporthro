...
class IndexStrassenCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
            ->setDescription('Reset\'s the solr index.')
            ->setName('hro:index:reset:strassen');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln('Indiziere Strassen fuer HRO-Adressensuche ... ');

        $stmt = $conn->query('SELECT count(*) AS count FROM regis.strassen_ohne_mehrfachnamen');
        $result = $stmt->fetch();
        $count = intval($result['count']);
        $stmt = $conn->query('SELECT count(*) AS count FROM regis.strassen_mehrfachnamen');
        $result = $stmt->fetch();
        ...
        while ($offset < $count) {
            $stmt = $conn->query("
                ...
                FROM regis.strassen_ohne_mehrfachnamen
                UNION SELECT uuid,
                gemeindeteil_name,
                gemeindeteil_name || ', ' || strasse_name AS strasse_name,
                ST_AsText(ST_Centroid(geometrie)) AS geom,
                ST_AsText(geometrie) AS wktgeom
                FROM regis.strassen_mehrfachnamen
                ORDER BY uuid
            ...
            while ($row = $stmt->fetch()) {
                ...
                /*$doc->phonetic = $this->addPhonetic($row['strasse_name']);*/

                $doc->label = "3".$row['strasse_name'];
