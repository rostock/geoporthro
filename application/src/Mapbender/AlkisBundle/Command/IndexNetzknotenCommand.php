...
class IndexNetzknotenCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
        ->setDescription('Reset\'s the solr index.')
        ->setName('hro:index:reset:netzknoten');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln('Indiziere Netzknoten fuer HRO-Strassennetzsuche ... ');


        $stmt = $conn->query('SELECT count(*) AS count FROM fachdaten.strassennetz_netzknoten_regis_hro');
        $result = $stmt->fetch();
            ...
            $stmt = $conn->query("
                SELECT
                 ...
                 ST_AsText(geometrie) AS wktgeom
                  FROM fachdaten.strassennetz_netzknoten_regis_hro
                   ORDER BY uuid
