...
class IndexBaumgruppeCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
        ->setDescription('Reset\'s the solr index.')
        ->setName('hro:index:reset:baumgruppen');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln('Indiziere Baumgruppen fuer HRO-Baumkatastersuche ... ');


        $stmt = $conn->query('SELECT count(*) AS count FROM fachdaten.baumgruppen_regis_hro');
        $result = $stmt->fetch();
        ...
        while ($offset < $result['count']) {
            $stmt = $conn->query("
                SELECT
                 ...
                 ST_AsText(geometrie) AS wktgeom
                  FROM fachdaten.baumgruppen_regis_hro
                   ORDER BY uuid
            ...
            while ($row = $stmt->fetch()) {
                ...
                $doc->label = "1".$row['bezirk'].$row['objektnummer'].$row['objektnummer_ohne_slashes'].$row['objektnummer_mit_leerzeichen'].$row['objektbezeichnung'].$row['teil'];
