...
class IndexGemeindeCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
        ->setDescription('Reset\'s the solr index.')
        ->setName('hro:index:reset:gemeinden');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln('Indiziere Gemeinden fuer HRO-Adressensuche ... ');

        $stmt = $conn->query("SELECT count(*) AS count FROM regis.gemeinden WHERE kreis_schluessel = '13003'");
        $result = $stmt->fetch();
        ...
        while ($offset < $result['count']) {
            $stmt = $conn->query("
                ...
                FROM regis.gemeinden WHERE kreis_schluessel = '13003' ORDER BY id LIMIT " . $limit . " OFFSET " . $offset);
            ...
            while ($row = $stmt->fetch()) {
                ...
                //$doc->phonetphonetic = $this->addPhonetic( $row['gemeinde_name']);
