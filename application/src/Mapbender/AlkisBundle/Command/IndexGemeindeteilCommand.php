...
class IndexGemeindeteilCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
            ->setDescription('Reset\'s the solr index.')
            ->setName('hro:index:reset:gemeindeteile');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln('Indiziere Gemeindeteile fuer HRO-Adressensuche ... ');

        $stmt = $conn->query("SELECT count(*) AS count FROM regis.gemeindeteile WHERE kreis_schluessel = '13003'");
        $result = $stmt->fetch();

        while ($offset < $result['count']) {
            $stmt = $conn->query("
                SELECT gemeindeteil_name,
                ST_AsText(ST_Centroid(geometrie)) AS geom,
                ST_AsText(geometrie) AS wktgeom
                FROM regis.gemeindeteile WHERE kreis_schluessel = '13003' ORDER BY id LIMIT " . $limit . " OFFSET " . $offset);
            ...
            while ($row = $stmt->fetch()) {
                ...
                $doc->label = "2".$row['gemeindeteil_name'];
