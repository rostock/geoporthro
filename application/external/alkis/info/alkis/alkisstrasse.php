...
$sql ="SELECT k.land, s.regierungsbezirk, s.kreis, s.gemeinde, s.lage, s.bezeichnung AS snam, ";
...
$sql.="WHERE s.gml_id = $1 AND f.endet IS NULL AND g.endet IS NULL AND s.endet IS NULL AND lo.endet IS NULL AND f.land = g.gemeindezugehoerigkeit_land[1] AND f.gemarkungsnummer = g.gemarkungsnummer AND lo.gml_id = ANY(f.zeigtauf) AND (lo.land = s.land AND lo.regierungsbezirk = s.regierungsbezirk AND lo.kreis = s.kreis AND lo.gemeinde = s.gemeinde AND lo.lage = s.lage) ";
...
$sql.="WHERE s.gml_id = $1 AND f.endet IS NULL AND g.endet IS NULL AND s.endet IS NULL AND lm.endet IS NULL AND f.land = g.gemeindezugehoerigkeit_land[1] AND f.gemarkungsnummer = g.gemarkungsnummer AND lm.gml_id = ANY(f.weistauf) AND (lm.land = s.land AND lm.regierungsbezirk = s.regierungsbezirk AND lm.kreis = s.kreis AND lm.gemeinde = s.gemeinde AND lm.lage = s.lage)";
