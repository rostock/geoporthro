...
class ApplicationController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        foreach ($applications as $application) {
            ...
            if (!$application->isListed() && $user && $user != 'anon.' && strpos($token, 'admin') === false) {
                continue;
            }
    ...
    /**
     * Toggle application list state.
     *
     * @ManagerRoute("/application/{slug}/liststate", options={"expose"=true})
     * @Method("POST")
     */
    public function toggleListStateAction($slug)
    {
        $application = $this->get('mapbender')->getApplicationEntity($slug);
        ...
        $requestedListState = $this->get('request')->get('liststate');
        $currentListState   = $application->isListed();
        $newListState       = $currentListState;

        switch ($requestedListState) {
            case 'enabled':
            case 'disabled':
                $newListState = $requestedListState === 'enabled' ? true : false;
                $application->setListed($newListState);
                $em->flush();
                $message  = 'List state switched';
                break;
            case null:
                $message  = 'No list state given';
                break;
            default:
                $message  = 'Unknown list state requested';
                break;
        }
        ...
        return new Response(
            json_encode(
                array(
                'oldListState' => $currentListState ? 'enabled' : 'disabled',
                'newListState' => $newListState ? 'enabled' : 'disabled',
                'message' => $message)
            ),
            200,
            array('Content-Type' => 'application/json')
