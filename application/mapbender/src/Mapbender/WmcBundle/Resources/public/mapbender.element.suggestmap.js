(function($){
    $.widget("mapbender.mbSuggestMap", {
        ...
        _setup: function(){
            ...
            //this.element.find('ul li').bind("load", $.proxy(self._suggestMap, self));
        },
        _getState: function(id){
            $.ajax({
                url: this.elementUrl + 'load',
                type: 'POST',
                // Request must be synchronous because LayerTree component needs the layers configuration
                // to be complete before it can safely initialize its DOM. It waits for the
                // mapbender.setupfinished event. If we made this request here async (again), expect
                // issues with SuggestMap especially in combination with multi-theme applications.
                async: false,
                data: {
                    _id: id
                },
