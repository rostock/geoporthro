...
//        Mapbender.source[src.type].onLoadStart(src);
    ...
    setLayerPositions: function(activeLayers) {
        var self = this;
        if (activeLayers && activeLayers.length > 0) {
            $.each( activeLayers, function( index, item ) {
                var source = self.getSource({
                    'id': item.sourceId
                });
                var mqLayer = self.map.layersList[source.mqlid];
                mqLayer.position(index + 20);
            } );
        }
    ...
    _moveSource: function(source, before, after) {
        ...
        if (old_pos === new_pos)
            return;
        if (new_pos > old_pos) {
            this.sourceTree.splice(new_pos - 1, 0, this.sourceTree.splice(old_pos, 1)[0]);
        } else {
