...
class MapFieldSubscriber implements EventSubscriberInterface
{
    ...
    public function preSetData(FormEvent $event)
    {
        ...
        // check if dpi is a float -> add precision = "decimal places";
        if (key_exists('dpi', $data) && floatval($data['dpi']) > floatval(floor($data['dpi']))) {
            $help                     = explode(".", strval(floatval($data['dpi'])));
            $dpi_options['precision'] = strlen($help[1]);
        }
        $form->add($this->factory->createNamed('dpi', 'number', null, $dpi_options));
    }
