...
Mapbender.Geo.WmsSourceHandler = Class({'extends': Mapbender.Geo.SourceHandler },{
    ...
    getPrintConfig: function(layer, bounds, scale, isProxy, imageSize){
        var url = isProxy ? Mapbender.Util.removeProxy(layer.getURL(bounds)) : layer.getURL(bounds);
        if (imageSize) {
            url = url.replace(/WIDTH\=[0-9]*\&/, 'WIDTH=' + imageSize.w + '&');
            url = url.replace(/HEIGHT\=[0-9]*$/, 'HEIGHT=' + imageSize.h);
        }
        var printConfig = {
            type: 'wms',
            url: url
